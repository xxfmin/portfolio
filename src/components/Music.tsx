  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div className="absolute inset-0 bg-black bg-opacity-40 rounded-sm opacity-0 group-hover:opacity-25 transition-opacity duration-300" />
      </Link>
      <div className="flex flex-col ml-2.5 sm:ml-3 space-y-0.5 text-white min-w-0 flex-1 pr-8 sm:pr-10">
        ...
        <h4 className="font-[450] text-[0.75rem] opacity-80">{song.artist}</h4>
      </div>
      <Link
        href={song.songUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="absolute top-2 sm:top-2.5 right-2 w-6.5 h-6.5 sm:w-5 sm:h-5 text-white hover:opacity-90 transition-[opacity,transform] duration-300 active:scale-[0.92] cursor-pointer z-10 after:content-[''] after:absolute after:-inset-2"
      >
